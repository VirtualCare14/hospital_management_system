...
function ClinicLoginForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!username.trim() || !password.trim()) {
      setError('Please enter both Clinic Login ID and password.');
      return;
    }
  ...
  return (
    <div className="bg-white/95 backdrop-blur-xl border border-orange-100 rounded-3xl p-8 shadow-xl shadow-orange-500/10">
      ...
      {sessionExpired && (
        <div className="mb-6 p-4 rounded-xl bg-amber-50 border border-amber-200 text-amber-900 text-sm flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs opacity-90">Your session has timed out. Please sign in again.</p>
          </div>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="e.g. clinic_doctor"
              className="w-full pl-11 pr-4 py-3 bg-orange-50/20 border border-orange-200 rounded-xl text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all duration-200 font-mono text-sm"
            />
          </div>
        </div>
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="••••••••"
              className="w-full pl-11 pr-11 py-3 bg-orange-50/20 border border-orange-200 rounded-xl text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all duration-200 font-mono text-sm"
            />
            <button
              type="button"
        ...
        {/* Hospital Code Toggle */}
        <div>
          <button
            type="button"
            onClick={() => setShowCodeField(!showCodeField)}
            className="text-xs text-orange-600 hover:text-orange-700 font-bold flex items-center gap-1.5 transition-colors focus:outline-none cursor-pointer"
          >
            <Building2 className="w-3.5 h-3.5 text-orange-500" />
            {showCodeField ? '- Remove Hospital Access Code' : '+ Specify Hospital Access Code (Optional)'}
          </button>
          ...
          {showCodeField && (
            <div className="mt-3">
              ...
              <input
                ...
                className="w-full px-4 py-2.5 bg-orange-50/20 border border-orange-200 rounded-xl text-slate-900 placeholder-slate-400 text-xs font-mono focus:outline-none focus:ring-2 focus:ring-orange-500"
              />
              <p className="text-[11px] text-slate-500 mt-1">
                Directly targets a specific hospital instance if configured.
              </p>
            </div>
          )}
        </div>
        ...
        <button
          type="submit"
          disabled={submitting}
          className="w-full py-3.5 px-4 bg-gradient-to-r from-orange-500 to-amber-600 hover:from-orange-600 hover:to-amber-700 active:scale-[0.99] text-white font-bold rounded-xl shadow-lg shadow-orange-500/20 flex items-center justify-center gap-2 transition-all duration-200 disabled:opacity-50 cursor-pointer text-sm"
        >
          {submitting ? (
            <>
      ...
      <div className="mt-6 pt-6 border-t border-orange-100 text-center">
        <p className="text-xs text-slate-500 font-medium">
          Credentials are configured by Hospital Administrator in Clinic Settings.
        </p>
      </div>
    </div>
  );
      ...
      <div className="w-full max-w-md px-6 py-8 relative z-10">
        ...
        <div className="flex flex-col items-center text-center mb-8">
          ...
          <p className="text-orange-600 font-bold text-xs tracking-wider uppercase mt-1 flex items-center gap-1.5">
            <ShieldCheck className="w-4 h-4 text-orange-500" /> Dedicated Clinic Portal
          </p>
        </div>
        ...
        <div className="text-center mt-8 text-xs text-slate-500 font-medium">
          © 2026 Medora 360 Clinic Management System. All rights reserved.
        </div>
      </div>
    </div>
  );
